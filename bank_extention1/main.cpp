...
void transaction(string file_name);
       ...
       account_number = mylibrary::read_string("Please enter account number:");
        for (stclient& client : vclients)
        {
            if (client.account_number == account_number)
            {
                print_client(client);
                found = true;
                int amount = mylibrary::read_num("Please enter deposti amount?");
...
void withdraw(string file_name)
{
    vector <stclient> vclients = get_clients(file_name);
    bool found = false;
    string account_number;
...
void show_clients_balance_header(short num)
{
...
void transaction_operation(string file_name, short op)
{
    switch (op)
    {
    case 1:
...
void transaction(string file_name)
{
    ...
    transaction_operation(file_name, op);
}
...
void bank(string file_name)
{
    ...
    do
    {
        ...
        num = mylibrary::read_num("Choose what do you want to do? [1 to 7]?");
        if (num != 7)
        {
        ...
        else
        {
            ...
            cout << "\nPress any key to go back to exit...";
            cin.get();
            return;
        }
    } while (num != 7);
